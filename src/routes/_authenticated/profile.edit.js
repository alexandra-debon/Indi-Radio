...
import { z } from "zod";
import { SocialLinksBar, SocialLinksEditor, sanitizeLinks, } from "@/components/social/SocialLinksBar";
...
function EditProfilePage() {
    ...
    useEffect(() => {
        ...
        setSocialLinks(sl && typeof sl === "object" ? sl : {});
    }, [profile]);
    if (!profile || !session)
    ...
    function handleAvatarChange(e) {
        ...
        if (file.size > 20 * 1024 * 1024) {
            toast.error("Image trop lourde (20 Mo max)");
            return;
        }
        if (avatarUrl) {
            setPendingFile(file);
      ...
      <form onSubmit={handleSubmit} className="card-brut space-y-5 p-4">
        <div className="flex items-center gap-4">
          <div className="size-20 shrink-0 overflow-hidden rounded-full border-2 border-border bg-muted">
            {avatarUrl ? (<img src={avatarUrl} alt="Avatar" className="size-full object-cover"/>) : (<div className="flex size-full items-center justify-center text-2xl font-black">
          ...
          <div className="space-y-2">
            ...
            {avatarUrl && (<Button type="button" variant="outline" size="sm" disabled={removing || uploading} onClick={() => setShowRemoveDialog(true)} className="text-destructive">
        ...
        <div className="space-y-1.5">
          ...
          <Textarea id="bio" value={bio} onChange={(e) => setBio(e.target.value)} maxLength={500} rows={4} placeholder="Quelques mots sur toi…"/>
          <p className="text-[11px] text-muted-foreground">{bio.length}/500</p>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="website">Site / lien</Label>
          <Input id="website" value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="https://…" inputMode="url"/>
        </div>
      ...
      <aside className="space-y-2 lg:sticky lg:top-4 lg:self-start">
        ...
        <div className="card-brut space-y-3 p-4">
          <div className="flex items-center gap-3">
            {avatarUrl ? (<img src={avatarUrl} alt="" className="size-16 rounded-full border-2 border-border object-cover"/>) : (<div className="grid size-16 place-items-center rounded-full border-2 border-border bg-muted text-base font-black uppercase">
          ...
          {website && /^https?:\/\/.+\..+/.test(website) && (<div className="inline-flex items-center gap-1.5 text-xs font-bold text-primary underline break-all">
              <Globe className="size-3.5 shrink-0"/> {website.replace(/^https?:\/\//, "")}
            </div>)}
          {!bio && !website && (<p className="text-[11px] italic text-muted-foreground">
              Ajoute une bio et un lien pour enrichir ton profil.
            </p>)}
          {Object.keys(sanitizeLinks(socialLinks)).some((k) => k !== "__order" && k !== "__labels") && (<div className="border-t border-dashed border-border/60 pt-2">
