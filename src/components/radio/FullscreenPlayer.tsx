import { Play, Pause, Radio, X, Maximize2, ChevronDown } from "lucide-react";
...
function FullscreenPlayer({ onClose }: { onClose: () => void }) {
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    panelRef.current?.style.setProperty("transform", `translateY(${delta}px)`);
    const opacity = Math.max(0.4, 1 - delta / (window.innerHeight * 0.6));
    panelRef.current?.style.setProperty("opacity", String(opacity));
  };
  ...
  const onPointerUp = () => {
    ...
    if (delta > window.innerHeight * 0.25) {
      ...
      panelRef.current?.style.setProperty("opacity", "1");
    }
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label={t("live.fullscreenTitle")}
    ...
    >
      <div
        ref={panelRef}
        className={`flex h-full w-full flex-col bg-background shadow-2xl transition-[transform,opacity] duration-300 ease-out ${
          isClosing ? "translate-y-full opacity-80" : "translate-y-0 opacity-100"
        }`}
      ...
      >
        ...
        <div
          data-drag-handle
          className="shrink-0 cursor-grab active:cursor-grabbing border-b border-border bg-card/95 backdrop-blur"
        >
